...
export function Reciters() {
  ...
  const [selectedReciter, setSelectedReciter] = useState<{ id: string; name: string; server: string; moshafName: string } | null>(null);
  ...
  const selectReciter = (r: any, moshaf: any) => {
    setSelectedReciter({ id: r.id, name: r.name, server: moshaf.server, moshafName: moshaf.name });
    setPhase('surahs');
  };
      ...
      <div className="h-screen flex flex-col max-w-lg mx-auto bg-background" dir="rtl">
        <div className="px-4 py-4 flex items-center gap-4 bg-card shadow-sm border-b border-border flex-shrink-0">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground">{reciters?.length ?? 0} قارئ</p>
          </div>
        </div>
                  ...
                  >
                    <div className="flex items-center gap-3 text-right">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{moshaf.name}</p>
                      </div>
                    </div>
    ...
    return (
      <div className="h-screen flex flex-col max-w-lg mx-auto bg-background" dir="rtl">
        <div className="px-4 py-4 flex items-center gap-4 bg-card shadow-sm border-b border-border flex-shrink-0">
          ...
          <div>
            <h1 className="font-bold text-lg">{selectedReciter?.name}</h1>
            <p className="text-xs text-muted-foreground">{selectedReciter?.moshafName}</p>
          </div>
        </div>
          ...
          <div className="space-y-2 pb-6">
            {Array.from({ length: 114 }, (_, i) => i + 1).map(num => (
              ...
              >
                ...
                <div className="flex-1">
                  <p className="font-bold">{SURAH_NAMES[num]}</p>
                  <p className="text-xs text-muted-foreground">{SURAH_AYAHS[num] ?? '?'} آية</p>
                </div>
                {audio.surahNum === num && audio.reciterId === selectedReciter?.id && (
  ...
  // ── PHASE: Full Player (Spotify-like) ──
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-b from-primary/30 to-background max-w-lg mx-auto" dir="rtl">
      ...
      <div className="flex items-center justify-between px-4 pt-12 pb-4">
        ...
        <p className="text-sm font-bold text-foreground/70">قيد التشغيل</p>
        <div className="w-9" />
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 gap-6">
        <div className="w-64 h-64 bg-gradient-to-br from-primary to-primary/50 rounded-3xl flex items-center justify-center shadow-2xl shadow-primary/30">
          <span className="text-white text-8xl font-serif">ق</span>
        </div>

        <div className="text-center w-full">
          <h2 className="text-2xl font-bold">سورة {audio.surahName}</h2>
          <p className="text-muted-foreground mt-1">{audio.reciterName}</p>
        </div>
      </div>
      ...
      <div className="px-6 pb-16">
        ...
        <div className="flex items-center justify-between">
          <button
            onClick={() => { if (audio.surahNum && audio.surahNum > 1) { const newNum = audio.surahNum - 1; audio.play({ reciterId: audio.reciterId, reciterName: audio.reciterName, serverUrl: audio.serverUrl, surahNum: newNum, surahName: SURAH_NAMES[newNum] ?? '' }); } }}
          ...
          <button
            onClick={() => { if (audio.surahNum && audio.surahNum < 114) { const newNum = audio.surahNum + 1; audio.play({ reciterId: audio.reciterId, reciterName: audio.reciterName, serverUrl: audio.serverUrl, surahNum: newNum, surahName: SURAH_NAMES[newNum] ?? '' }); } }}
            className="p-4 bg-secondary rounded-full text-foreground"
